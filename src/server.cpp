...
int main()
{
    ...
    svr.Get("/query/heavy", [&](const httplib::Request& req, httplib::Response& res)
    {
        ...
        {
            ram_ids.push_back(log.id);
        }

        vector<int> dur_ids;
        for (const auto& log : dur_logs)
        {
            dur_ids.push_back(log.id);
        }
    ...
    svr.Post("/log", [&](const httplib::Request& req, httplib::Response& res)
    {
        ...
        if (req.has_param("func"))
        {
            ...
            int new_id = trace_db->log_event(func.c_str(), msg.c_str(), dur, ram, rom);

            cout << "Log saved. ID: " << new_id << endl;
    ...
    svr.Get("/query", [&](const httplib::Request& req, httplib::Response& res)
    {
        ...
        cout << "Querying ID range: " << start << " to " << end << endl;
    ...
    svr.Get("/query/ram", [&](const httplib::Request& req, httplib::Response& res)
    {
        ...
        cout << "Querying RAM range: " << min_ram << " to " << max_ram << endl;
    ...
    svr.Get("/query/duration", [&](const httplib::Request& req, httplib::Response& res)
    {
        ...
        cout << "Querying Duration range: " << min_dur << " to " << max_dur << endl;
    ...
    cout << "Shutting down databases..." << endl;
    delete trace_db;
    delete auth_db;
    return 0;
}
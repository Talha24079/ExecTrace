...
void register_user(const string& username)
{
    ...
    if (res && res->status == 200) {
        try {
            ...
            cout << "Success! API Key: " << json_res["api_key"].get<string>() << endl;
        } catch (...) {
            cout << "Error: Invalid JSON response." << endl;
        }
    } else {
...
void query_logs(const string& api_key, const string& type, int min_val, int max_val)
{
    ...
    if (res && res->status == 200) {
        try {
            ...
            {
                // UPDATED: Now prints User ID and Version
                cout << " - [ID:" << log["id"] << "] " 
                     << "User:" << log["user_id"] << " "
                     << "(v" << log["version"] << ") " 
                     << log["func"] 
                     << " | RAM: " << log["ram"] 
                     << " | Dur: " << log["duration"] << endl;
            }
...
int main(int argc, char* argv[])
{
    ...
    if (command == "register" && argc == 3) register_user(argv[2]);
    else if (command == "log" && argc == 6) log_trace(argv[2], argv[3], stoi(argv[4]), stoi(argv[5]));
    else if (command == "query" && argc == 6) query_logs(argv[2], argv[3], stoi(argv[4]), stoi(argv[5]));
